...
| 2026-09-01 | [NPM_ROTO_Y_REGLA_DE_VERSION.md](2026-09-01/NPM_ROTO_Y_REGLA_DE_VERSION.md) | El `package-lock.json` iba **nueve versiones por detrás** (0.1.55 contra 0.1.64), y el motivo es que **`npm ci` y `npm install` no funcionan**: ERESOLVE por los peers exactos de tiptap, con el conflicto en el propio `package.json`. `pnpm install --frozen-lockfile` sí. **El `CLAUDE.md` decía «usa npm» y era falso** — corregido. Y la regla de versionado que faltaba, con el paso 3 que se olvida: actualizar en los consumidores. | ✅ versión corregida y regla escrita · 🟡 D-2 se inclina: el lockfile roto es el de npm | 2026-09-01 · `a6d0c29` · npm 0.1.64 = main 0.1.64 · `npm ci` ✗, `pnpm --frozen-lockfile` ✓, `build:lib` ✓ |
